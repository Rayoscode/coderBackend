...
export default class ContenedorCarritos {
    ...
    async save(carrito:Carrito){
        ...
        try{
            ...
            const obj = {id:max + 1, timestamp:carrito.timestamp, productos:carrito.productos};
            objetos.push(obj)
    ...
    async upgradeByID(carritoAModificar:Carrito){
        let objetos = await this.getAll();
        objetos.forEach((obj )=>{
            if(obj.id === carritoAModificar.id){
                obj.productos = carritoAModificar.productos;
                obj.timestamp = carritoAModificar.timestamp;
            }
        })
        this.deleteAll();
        try{
...
const contenedor:ContenedorCarritos = new ContenedorCarritos("../productos.txt");